...
export function renderSkill(serverUrl = "https://ops.<zone>"): string {
  ...
  return [
    ...
    "",
    "One plane serves the whole fleet: every tool takes an optional",
    "`project` naming an enrolled project (see `fleet_projects`); omitted,",
    "the plane's default project applies. Audit rows record the resolved",
    "project, never the word default.",
    "",
